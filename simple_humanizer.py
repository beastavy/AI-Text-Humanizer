...
def expand_contractions(text):
    """Expand common contractions in a robust way"""
    contractions = {
        ...
        "how's": "how is", "let's": "let us", "that's": "that is",
...
def add_academic_transitions(text):
    """Add academic transitions between sentences more intelligently"""
    transitions = [
    ...
    if len(sentences) > 1:
        for i in range(1, len(sentences)):
            ...
            if (random.random() < 0.3 and  # 30% chance to add transition
                not sentence.startswith(('Moreover,', 'Furthermore,', 'Additionally,',
                                       'However,', 'Nevertheless,', 'Therefore,',
                                       'Hence,', 'Consequently,', 'And', 'But', 'Or', 'So'))):
                transition = random.choice(transitions)
                sentences[i] = f"{transition} {sentence}"
    ...
    for i, sentence in enumerate(sentences):
        if i > 0 and sentence.strip():
            ...
            if random.random() < 0.2:
                connectors = ["This", "Such", "These", "That", "It", "However"]
                if not sentence.startswith(tuple(connectors)) and not sentence[0].isupper():
                    sentence = f"{random.choice(connectors)} {sentence.lower()}"
                elif sentence[0].isupper():
...
def humanize_text(text, use_transitions=True, use_structure=True):
    """Enhanced text humanization with better error handling"""
    ...
    try:
        ...
        st.error(f"Error in humanization: {e}")
        return text
