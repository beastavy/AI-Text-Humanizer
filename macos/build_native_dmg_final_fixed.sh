...
# AI Text Humanizer Pro - Native macOS DMG Builder (Final Fix)
...
a = Analysis(
    ...
    datas=[
        ...
        ('$PROJECT_ROOT/nltk_data', 'nltk_data'),
    ],
    hiddenimports=[
        'nltk',
        'spacy',
...
if [ -f "$ICON_ICNS" ]; then
  ...
  # Copy icon to Resources (avoiding conflict with icon file)
  cp "$ICON_ICNS" "$RES_DIR/app_icon.icns"

  # Update Info.plist to reference the icon
  PLIST="$APP_PATH/Contents/Info.plist"
  if command -v /usr/libexec/PlistBuddy >/dev/null 2>&1; then
    /usr/libexec/PlistBuddy -c "Add :CFBundleIconFile string app_icon.icns" "$PLIST" 2>/dev/null || \
    /usr/libexec/PlistBuddy -c "Set :CFBundleIconFile app_icon.icns" "$PLIST" 2>/dev/null || true
  fi
fi
